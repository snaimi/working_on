...
class ZoneFormatter:
    ...
    def _extract_metadata_dict(self, zone_obj: Any) -> Dict[str, Any]:
        """Safely extracts metadata as a dictionary regardless of underlying object type."""
...
class DynamicDroneSimulation:
    ...
    def plan_move(
        ...
        reserved_capacities: Dict[str, int],
        zone_capacities: Dict[str, int],
    ) -> None:
        """Phase 1: Decide next destination strictly progressing forward towards goal."""
        ...
        for child in current_node.child:
            ...
            # Direct Forward Movement Enforcement
            ...
            # Check capacity including turn reservations
            effective_occupancy = occupancy_tracker.get(child, 0) + reserved_capacities.get(child, 0)
            max_capacity = zone_capacities.get(child, 1)

            if effective_occupancy >= max_capacity:
                continue
        ...
        if best_neighbor is not None:
            ...
            reserved_capacities[best_neighbor] = reserved_capacities.get(best_neighbor, 0) + 1

    def execute_move(self, occupancy_tracker: Dict[str, int]) -> Optional[str]:
        """Phase 2: Advance physical movement based on step traversal cost."""
        if self.finished:
            return None
        ...
        # Reserve occupancy inside tracker upon embarking
        if self.cost_accumulated == 0.0:
            occupancy_tracker[self.target_zone] = occupancy_tracker.get(self.target_zone, 0) + 1
...
class SimulationManager:
    ...
    def __init__(self, total_drones: int, zones: List[Any]) -> None:
        ...
        self.display_turn_num: int = 1  # Started turn counter at 1
        self.sleep_delay: float = 0.0 if total_drones > 100 else 0.1

        # Resolve Start/Goal names dynamically
    ...
    def _spawn_drones(self) -> None:
        """Lazy-instantiate drones when capacity in the start zone opens up."""
    ...
    def _plan_phase(self) -> None:
        """Instructs all active drones to plan their next step."""
        reserved_capacities: Dict[str, int] = {}
    ...
    def _execute_phase(self) -> None:
        """Executes drone movements, prints turn logs, and garbage-collects completed drones."""
        ...
        # Remove finished drones immediately
