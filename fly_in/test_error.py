...
for file_path in folder_path.iterdir():
    with open(file_path) as f:
        for line in f:
            # print(f"entire line : {line.strip()}")
            if ":" in line and not line.startswith("#"):
                first_line += 1
                #
                # need to modify the i possibly to
                # line.split(":")[0] != "nb_drones":
                #
                # -> test
                ls_line = line.split(":")
                # changed to ->
                ...
                if line.split(":")[0] != "nb_drones":
                    if ls_line[0].strip() != "connection":
                        ...
                        ###############################
                        if len(ls_ls_dtls) != 3:
                            sys.stderr.write("Presence of a space charachter : error \n")
                        ...
                        for meta_item in ls_ls_meta:
                            try:
                                ...
                                sys.stderr.write(f"No value present : error -> {e}\n")
                            if key == "max_drones":
                        ...
                        try:
                            ...
                            sys.stderr.write(f"invalid literal for int() with base 10 : error {e}\n")
                    else:
                        # if the condition is True -> ls_line[0].strip() == "connection":
                        ...
                        if len(connections_ls) != len(connections_set):
                            sys.stderr.write("Presence of a dup connections : error \n")
                        connections_ls_reverse = [i[::-1] for i in connections_set]
                        connections_ls_all = connections_ls + connections_ls_reverse
                        connections_set_all = set(connections_ls_all)
                        # reversed connection
                        if len(connections_set_all) != (len(connections_ls)*2):
                            sys.stderr.write("Presence of a reversed connection : error \n")
                        # ##############################
                        # A connection references a zone name that wasn't defined yet
                        if start_point not in names_ls or end_point not in names_ls:
                            sys.stderr.write("zone name wasn't defined yet : error \n")
                        # ##############################
                        try:
                ...
                if line.split(":")[0] == "nb_drones":
                    try:
                        ...
                        if i > 1:
                            raise ValueError(
                                "Frequency Error: 'nb_drones' appeared" f" {i} times"
                            )
                        if first_line != 1:
                ...
                #
                    ...
                    try:
                        ...
                        print(f"invalid file form : error in the start_hub field -> {e}")
                #
                ...
                #
                    ...
                    try:
                        ...
                        print(f"invalid file form : error in the end_hub field -> {e}")
                #
                ...
                #
                    ...
                    try:
                        ...
                        print(f"invalid file form : error in the connection field -> {e}")
            else:
                continue
